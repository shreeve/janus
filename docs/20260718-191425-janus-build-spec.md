...
- Cold capability **control** (see [`20260718-203749-capability-control.md`](20260718-203749-capability-control.md)): `internal` / `local` / `public`
- Global Caddyfile: `janus { control local }` (process-wide); site `janus` admits data plane
- Serve Incus-inspired `/1.0` + `/1.0/health` on configured control listeners
- Sync responses; structured errors
- **Accept:** `curl http://127.0.0.1:7600/1.0` (local) and/or `curl --unix-socket …` (internal); public ping still works

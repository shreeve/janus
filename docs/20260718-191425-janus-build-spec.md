...
| 1 | **ping** | Phase 1 |
| 2 | **control** | Phase 2 |
| 3+ | (none yet — next work is hot `/1.0` apps, then upstreams, …) | Phase 3+ |
...
- `PUT …/upstreams` is an atomic full-list swap; the supervisor serializes its own writes (awaits each 200), so no generation fencing on the wire (see [pool protocol](20260719-002000-pool-protocol.md))
...
Implement **Phase 3** — in-memory apps registry on `/1.0/apps`.

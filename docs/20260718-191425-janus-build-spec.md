...
Every numbered phase is complete: Phase 7 (Hub) shipped at `919c4bd`
with its measurements recorded.
...
- [x] Measure fan-out throughput/latency, idle cost, slow-consumer
  isolation, reload behavior, and text-bridge tax — the six-measurement
  entry lives in the perf doc's Measured results ("Hub: the six Phase 7
  measurements", raw provenance
  [`20260720-214446-bench-raw-hub.txt`](20260720-214446-bench-raw-hub.txt),
  implementation commit `919c4bd`): sustained fan-out ~0.4M
  deliveries/s roughly independent of room size, p99 flat until
  saturation, ~68KB per idle conn at ~2,700–3,000 admitted conns/s,
  wedged consumer closes 1013 with others' p99 unmoved, reload under
  fan-out drops zero sockets, and edge send throughput does not degrade
  behind a slow tenant
...
Every phase and acceptance box above is ticked. What remains is
deliberate follow-on work, not this SPEC's scope: the two pending
owner policy calls (a `docs/bench/` subfolder for raw bench files;
per-app cache byte quotas, open question Q7) and the deferred
performance levers gated on a real production tenant.

...
| 4 | **hub** | Phase 7 design contract; implementation pending |
...
- [ ] Targeted re-review confirms the revised contract before implementation
- [ ] Site-scoped cold `hub` capability with per-app WebSocket state
  reused across Caddy reload through `caddy.UsagePool`
- [ ] One JSON object-or-list grammar across client, bridge-response,
  and publish planes, with a per-plane policy table
- [ ] `@` addresses event delivery; `/` entries are channels and other
  strings are direct connection ids; hierarchy is naming-only
- [ ] Client `+`/`-` mutate only the sender; cross-connection mutation
  and channel expansion as a membership subject are trusted-plane-only
- [ ] Array-valued `<` provenance is edge-stamped as
...
- [ ] Open admission atomically reserves `max_conns`, rechecks the hub
  tombstone after the bridge response, and starts the reader only
  after registration and open-response processing
- [ ] Bridge POSTs to hot `bridge_path`: open is synchronous admission,
...
- [ ] Control `POST /1.0/apps/{id}/hub/publish`, opaque/count-only
  membership snapshot, and explicit app-ownership checks when
  host-claim scoping exists
- [ ] Whole-frame validation rejects every violation before mutation
  or delivery; bounded outbound queues disconnect slow consumers
- [ ] **Accept:** `test.sh` Hub group pins every contract row,

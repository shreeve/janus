...
const appsFixture = `[{"id":"a1","name":"shop","hosts":["shop.via.rip","shop.local"],"upstreams":[{"path":"/tmp/w1.sock"},{"path":"/tmp/w2.sock"}],"lease":"heartbeat"},` +
	...
	`{"id":"c3","name":"medlabs","hosts":[],"upstreams":[{"path":"/tmp/bell.sock","doorbell":true}],"site":{"host":"{site}.medlabs.health","dir":"/srv/medlabs/sites","aliases":{"z.local":"z","a.local":"a"}},"lease":"heartbeat"}]`
...
func TestAppsVerb(t *testing.T) {
	...
	for _, want := range []string{"3 apps registered (control unix " + p.sock + ")", "NAME", "shop", "shop.via.rip shop.local", "2 workers", "heartbeat", "browse /Users/ann/docs", "process", "doorbell /tmp/bell.sock, sites in /srv/medlabs/sites", "{site}.medlabs.health a.local z.local", "c3"} {

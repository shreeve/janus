...
//
// In wan mode the local name families — <name>.local, <name>.localhost,
// and localhost — are not served, not certified, and not announced. They
// are names for a machine you sit at and a network you are on; a public
// address has neither, and a request that carries one in its Host header
// is someone asking the wrong question.
...
func localFamilyHost(host string) bool {
	return host == "localhost" || strings.HasSuffix(host, ".local") || strings.HasSuffix(host, ".localhost")
}
